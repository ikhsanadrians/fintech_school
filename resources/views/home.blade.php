                ...
                <div class="flex flex-col basis-[25%] mt-2">
                    ...
                    <div class="flex flex-col gap-3 border border-slate-300 rounded-xl p-4">
                        ...
                        @if ($product_hapus == [])
                            ...
                            @endforeach
                        @else
                            @foreach ($product_hapus as $ts)
                                <div class="flex items-center gap-2 w-full">
                                    @if ($ts->stock == 0)
                                        <del class="w-full"> <span class="w-full">{{ $ts->name }} |
                                                {{ $ts->price }}
                                                ({{ $ts->quantity }})
                                            </span>
                                        </del>
                                    @else
                                        <span class="w-full">{{ $ts->name }} | {{ $ts->price }}
                                            ({{ $ts->transaction->quantity }})
                                        </span>
                                    @endif
                                ...
                                @php
                                    $totalPrice += $ts->price * $ts->transaction->quantity;
                                @endphp
                            @endforeach
                        @endif
